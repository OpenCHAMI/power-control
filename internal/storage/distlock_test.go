...
func (s *StorageTestSuite) TestPing() {
	err := s.dlp.Ping()
	s.Require().NoError(err, "DistLock Ping() failed")
}
...
func (s *StorageTestSuite) TestDistributedLock() {
	...
	s.Require().NoError(err, "DistributedTimedLock() failed")

	time.Sleep(1 * time.Second)
	if s.dlp.GetDuration() != lockDur {
		t.Errorf("Lock duration readout failed, expecting %s, got %s",
			lockDur.String(), s.dlp.GetDuration().String())
	}
	err = s.dlp.Unlock()
	s.Require().NoErrorf(err, "Error releasing timed lock (outer): %v", err)

	if s.dlp.GetDuration() != 0 {
